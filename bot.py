...
async def get_swap_route(input_mint, output_mint, amount, slippage=3):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(JUPITER_API_URL, params=params, timeout=30) as resp:
                if resp.status != 200:
                    raise Exception(f"Jupiter API returned status {resp.status}")
                data = await resp.json()
                routes = data.get("data")
                if not routes:
                    raise Exception(f"No routes returned from Jupiter for {output_mint}")
                if "swapTransaction" not in routes[0]:
                    raise Exception(f"No swapTransaction in route for {output_mint}")
                return routes[0]
        except Exception as e:
...
async def execute_buy(mint, amount_usd=None):
    ...
    try:
        async with AsyncClient(RPC_URL) as client:
            ...
            logging.info(f"Successfully bought {mint}: {sig}")
        return True, sig
    except Exception as e:
